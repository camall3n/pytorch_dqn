...
class SMAC:
    def __init__(self, input_shape, action_space, device, args):
        ...
        self.encoder, self.feature_size = build_phi_network(args, input_shape)
        ...
        self.markov_head = MarkovHead(args, self.feature_size, action_space.shape[0])
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        state_batch = torch.as_tensor(minibatch.state).float().to(self.device)
        next_state_batch = torch.as_tensor(minibatch.next_state).float().to(self.device)
